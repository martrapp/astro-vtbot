...
```
The `<AnimationStyle/>` component can be placed in the `<head>` or in the `<body>` in your component template. Even if it is not placed in the `<head>`, the generated CSS will appear in the `<head>`. 
