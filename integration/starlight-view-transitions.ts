	...
	return {
		...
		hooks: {
			'config:setup'(sl: HookParameters<'config:setup'>) {
				...
				sl.astroConfig.integrations.find(({ name }) => name === 'astro-vtbot') || sl.addIntegration(vtbot());
				sl.addRouteMiddleware({ entrypoint: "astro-vtbot/integration/starlight-middleware" });
				const css = resolve(fileURLToPath(import.meta.url), '../../styles/starlight-view-transitions.css');
				sl.updateConfig({
