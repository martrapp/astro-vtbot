...
...
```
Now you can use other direction names than `forwards` and `backwards` in your animation. Make sure to select proper direction values by setting `e.direction` to one of your custom direction names in the event handler for `astro:before-preparation`. See this in action in the [FishPond Tech Demo](/fish-pond/center/)
